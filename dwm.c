...
drawstatusbar(Monitor *m, int bh, char* stext) {
	// HACK: Weirdass padding stuff because I have no idea what's going on here
	int ret, i, w, x, len;
	short isCode = 0;
	char *text;
	char *p;
	...
	drw->scheme[ColFg] = scheme[ m == selmon && m->sel ? SchemeSel : SchemeNorm][ColFg];
	drw->scheme[ColBg] = scheme[ m == selmon && m->sel ? SchemeSel : SchemeNorm][ColBg];
...
drawbar(Monitor *m)
{
	...
	tw = m->ww - drawstatusbar(m, bh, stext) - 1; // Weirdass -1 offset because my merge of status2d and barpadding
...
updatepreview(void)
{
	...
	for (m = mons; m; m = m->next) {
		m->tagwin = XCreateWindow(dpy, root, m->wx, m->by + bh + vertpad * 2, m->mw / scalepreview, m->mh / scalepreview, 0,
